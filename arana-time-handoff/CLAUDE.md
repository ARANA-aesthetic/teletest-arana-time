...
### แท็บ "รายงาน" (Reports) — เพิ่มใหม่
แท็บที่ 5 ใน bottom nav แอดมิน — เลือกเดือน (‹ ›) + ฟิลเตอร์สาขา + สลับ 4 มุมมองด้วย segmented bar:
`ขาด-ลา` / `มาสาย` / `OT` (ทั้ง 3 เป็นตารางรายวัน แถว=พนักงาน คอลัมน์=วันที่ 1..31) และ `สรุปเดือน` (รวมรายคน แยกตามประเภทการลา)
- ดึงข้อมูลสดจาก `db.leaves`/`db.logs`/`db.otRequests` ที่โหลดไว้แล้วใน client ไม่ต้องยิง query เพิ่ม
- ฟังก์ชันหลัก: `buildMonthReport()` (รวบรวมข้อมูลก้อนเดียวใช้ร่วมกันทั้งจอและการส่งออก) → `dailyTableHTML()` / `summaryTableHTML()` / `reportSheetPayload()`
- ปุ่ม "ส่งรายงานเดือนนี้ไป Google Sheets" ยิง POST ตรงจาก browser ไปที่ Apps Script Web App (ไม่ผ่าน Supabase) ใช้ `Content-Type: text/plain` เพื่อเลี่ยง CORS preflight — สร้าง/เขียนทับ 4 แท็บ: `รายงานลา YYYY-MM` / `รายงานสาย` / `รายงาน OT` / `สรุปเดือน` (ปี พ.ศ.)
- ฝั่ง Apps Script: ฟังก์ชัน `writeReportSheet()` จะเขียนทับได้เฉพาะแท็บที่มี note `ARANA_REPORT` ที่ A1 (แท็บที่สคริปต์สร้างเอง) ถ้าชื่อซ้ำกับแท็บที่ผู้ใช้ทำเอง จะสร้างแท็บใหม่ต่อท้าย " (แอป)" แทน ไม่ลบข้อมูลเดิมเด็ดขาด

### หน้าตาแอป Admin (bottom nav 6 แท็บ)
