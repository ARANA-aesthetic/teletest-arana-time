-- ARANA TIME — แก้ sync_to_google_sheets() ให้ใช้ pg_net แทน extension "http"
-- รันทั้งไฟล์นี้ใน Supabase SQL Editor ครั้งเดียว (ปลอดภัย รันซ้ำได้ ใช้ CREATE OR REPLACE)
--
-- สาเหตุที่ต้องแก้: Google Apps Script Web App ตอบกลับด้วย HTTP 302 redirect ไปที่
-- script.googleusercontent.com เพื่อส่ง response จริง (พฤติกรรมมาตรฐานของ Apps Script)
-- extension "http" (ของ pramsey) ที่ใช้ในไฟล์ gsheet-realtime-backup.sql เดิม จัดการ
-- POST + redirect แบบนี้ไม่ได้ดี ทำให้เกิด error "HTTP request cancelled"
--
-- เปลี่ยนมาใช้ pg_net (ตัวเดียวกับที่ใช้ส่ง Telegram อยู่แล้วในโปรเจกต์นี้ พิสูจน์แล้วว่าทำงานได้จริง
-- ในฐานข้อมูลนี้) — ยิง request แบบ async แล้ว poll ผลลัพธ์จากตาราง net._http_response เอง
-- (รอสูงสุด ~6 วินาที) แทนการรอผลทันทีแบบ extension http เดิม
...
begin
  ...
  -- ยิง request แบบ async ผ่าน pg_net แล้ว poll ผลลัพธ์เองจาก net._http_response
  -- (สูงสุด 12 ครั้ง ห่างครั้งละ 0.5 วิ = รอไม่เกิน ~6 วินาที)
  ...
  for i in 1..12 loop
    ...
    select status_code, convert_from(content, 'UTF8') into v_status, v_content
