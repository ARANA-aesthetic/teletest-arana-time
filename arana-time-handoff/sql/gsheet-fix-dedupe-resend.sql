-- ARANA TIME — แก้ backup Google Sheets ทุก 15 นาที ที่ข้อมูลซ้ำซ้อนกันเป็นก้อนใหญ่ต่อเนื่อง
-- รันทั้งไฟล์นี้ใน Supabase SQL Editor ครั้งเดียว (ปลอดภัย รันซ้ำได้ — create or replace)
--
-- ปัญหาที่ user แจ้ง: ทุก 15 นาที ระบบไม่ได้ส่งเฉพาะรายการใหม่ แต่ยกข้อมูลทั้งก้อนมาวางซ้ำต่อกันเรื่อยๆ
--
-- สาเหตุที่พบ (2 ชั้น ต้องแก้ทั้งคู่):
--
-- 1) ฝั่ง Postgres (gsheet-fix-async-nowait.sql เดิม) — เป็น fire-and-forget แบบ 2 ขั้นตอนในฟังก์ชัน
--    เดียวกัน: ขั้นที่ 1 เช็คผลลัพธ์ของ batch ก่อนหน้า, ขั้นที่ 2 ดึงแถวที่ synced_at is null ไปส่งเป็น
--    batch ใหม่ — แต่ขั้นที่ 2 ไม่เช็คว่าแถวไหน "ถูกส่งไปแล้วใน batch ก่อนหน้าที่ยังไม่ได้รับผลตอบกลับ"
--    (v_status ยังเป็น null เพราะ pg_net ยังไม่ตอบ) ทำให้ถ้าฟังก์ชันถูกเรียกซ้อนกันถี่ๆ ในช่วงเวลาสั้นๆ
--    (เช่น กดปุ่ม "ทดสอบ/Sync ตอนนี้" ในแอปหลายครั้งติดกัน) แถวชุดเดิมจะถูกดึงไปส่งซ้ำเป็น batch ใหม่ทับ
--    ของเดิมที่ยังค้างอยู่ — แก้โดยจำ id ของ batch ที่ "ยังไม่ได้รับผลตอบกลับ" ไว้ แล้วกันไม่ให้ขั้นที่ 2
--    ดึงแถวชุดเดิมไปส่งซ้ำในรอบเดียวกัน
--
-- 2) ฝั่ง Google Apps Script (arana-sheets-sync.gs) — appendRows() เดิม "ต่อท้ายทุกแถวที่ได้รับ" ไม่มี
--    การเช็คว่า id (leave_id/log_id/ot_id คอลัมน์รองสุดท้ายของทุกชีท) เคยถูกบันทึกไปแล้วหรือยัง — นี่คือ
--    จุดที่ทำให้ "เห็น" ข้อมูลซ้ำในชีทจริงๆ ไม่ว่าฝั่ง Postgres จะมีบั๊กหรือไม่ก็ตาม (เช่น เน็ตหลุดกลางทาง
--    ทำให้ Postgres ไม่เห็น response แล้วส่งซ้ำ, หรือกด sync ซ้อนกัน) — ต้องอัปเดตไฟล์ .gs แล้ว "Deploy
--    เวอร์ชันใหม่" ด้วย (แก้แค่โค้ดในสคริปต์ไม่พอ ต้อง Deploy → Manage deployments → New version)
--    ไฟล์ .gs ในโฟลเดอร์นี้แก้ไว้ให้แล้ว ให้คัดลอกไปวางทับในโปรเจกต์ Apps Script แล้ว deploy ใหม่
--
-- ชั้นที่ 2 (Apps Script dedupe) คือตัวการันตีจริงว่าชีทจะไม่มีแถวซ้ำอีกต่อไป ไม่ว่าฝั่ง Postgres จะพลาด
-- ยังไงก็ตาม — ชั้นที่ 1 (Postgres) เป็นการลดการส่งซ้ำที่ไม่จำเป็นเฉยๆ ไม่ใช่ตัวกันซ้ำหลัก
...
declare
  ...
  v_pending_log_ids text[] := '{}';
  v_pending_ot_ids text[] := '{}';
  v_payload jsonb;
  v_request_id bigint;
  v_status int;
  v_content text;
...
begin
  ...
  for v_batch in select * from gsheet_sync_batches where resolved = false order by created_at loop
    ...
    if v_status = 200 then
      ...
      -- ยังไม่ได้รับผลตอบกลับ (pg_net ยังไม่ตอบ) — ★ กันไม่ให้ขั้นที่ 2 ดึงแถวชุดนี้ไปส่งซ้ำในรอบนี้ ★
      v_pending_leave_ids := v_pending_leave_ids || v_batch.leave_ids;
      v_pending_log_ids := v_pending_log_ids || v_batch.log_ids;
      v_pending_ot_ids := v_pending_ot_ids || v_batch.ot_ids;
    end if;
  end loop;

  -- ===== ขั้นที่ 2: รวบรวมแถวที่ยังไม่ sync และไม่ได้ค้างอยู่ใน batch ที่ยังรอผลอยู่ แล้วยิง request ใหม่ =====
  for rec in
    ...
    where l.synced_at is null
      and not (l.id = any(v_pending_leave_ids))
  ...
  for rec in
    ...
    where lg.synced_at is null and lg.type in ('IN','OUT')
      and not (lg.id = any(v_pending_log_ids))
    order by lg.time
    limit 1000
  loop
  ...
  for rec in
    ...
    where o.synced_at is null and o.status = 'approved'
      and not (o.id = any(v_pending_ot_ids))
...
-- ★★★ อย่าลืมขั้นตอนที่ 2: เอาไฟล์ arana-sheets-sync.gs (แก้ dedupe แล้ว) ไปวางทับในโปรเจกต์ Apps Script
-- เดิม แล้ว Deploy → Manage deployments → แก้ไข (ไอคอนดินสอ) → เปลี่ยน Version เป็น "New version" → Deploy
-- ถ้าไม่ deploy เวอร์ชันใหม่ โค้ดเดิมที่ไม่มี dedupe จะยังทำงานอยู่ ต่อให้แก้ไฟล์ .gs ในเครื่องแล้วก็ตาม ★★★
